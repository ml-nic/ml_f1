...
def grab_screen(scale=1.0, region=None, window_title='Game'):
    ...
    :parameter window_title: string to search title bars for. Defaults to 'F1'
    """
    global a
    ...
    if grab_screen.screen is None or grab_screen._region != region or grab_screen._scale != scale:
        ...
        if region is None:
            grab_screen.screen = windows_screen_grab(window_title, scale=scale)
        else:
            grab_screen.screen = windows_screen_grab(window_title, scale=scale, region=region)
...
class windows_screen_grab:
    ...
    def get_screen_bits(self):
        self._memDC.StretchBlt((0, 0), (self._dest_width, self._dest_height), self._windowDC, (self._top, self._left),
